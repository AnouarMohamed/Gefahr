...
| Kubernetes baseline | Shipped | A hardened manifest includes secret-backed admin auth, exec probes, read-only non-root pods, restricted admin networking, services, PodDisruptionBudget, rollout safety settings, and a documented multi-replica local-state contract. |
...
| Distributed cluster state | Kubernetes replicas are supported as stateless data-plane pods, but cache entries, route rate-limit counters, backend health observations, balancer counters, reload snapshots, and process metrics are per pod. There is no distributed cache, global rate-limit backend, EndpointSlice watcher, or shared health coordinator. |
